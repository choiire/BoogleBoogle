...
| `CONFIG_OBJECT_BUBBLE_MAX` | `20` | 최대 동시 버블 수 |
...
| `KEY_SPACE` | — | — | — | `ATTACK` | `shot_timer ≤ 0`일 때 `bubble_add()` 호출 |

**Logic & Notes**
- `player->obj.is_active == false`이면 즉시 반환 (사망 상태 보호)
- 버블 발사 성공 시 `shot_timer = 20`, `attack_timer = 10` 설정
...
|---|---|---|---|---|
| `BASIC` | 지상 추적 + 확률적 점프 | 없음 (접촉 피해) | 1.2 px/f | 2.4 px/f |
| `THROW` | 공중 부유 (중력 무시) | 투사체 발사 | 0.8 px/f | 1.6 px/f |
...
- 분노 상태에서는 이동 속도가 증가합니다 (`BASIC`: 2.4, `THROW`: 1.6).
